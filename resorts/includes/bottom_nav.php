<div class="container explore mb-4">
    <div class="row pb-2">
      <div class="col-2 pr-0">
        <h3 class="greyFooter pl-0 explore-heading" style="font-size: 15px;">EXPLORE QVC</h3>
      </div>
      <div class="col-9 pl-5">
        <h3 class="greyFooter pl-0 explore-heading" style="font-size: 15px;">QVC CLUB DESTINATIONS</h3>
      </div>
    </div>
    
    <div class="row d-flex">
        <div class="explore-column" style="background-color:#dfdfdf;">
          <!-- <h3 class="greyFooter">EXPLORE QVC</h3> -->
          <p class="footerText">
            <a href="../resort_portfolio.php" class="footerLinks">Resort Portfolio</a><br>
              <a href="../our_club.php" class="footerLinks">Our Club</a><br>
              <a href="../feedback.php" class="footerLinks">Member Feedback</a><br>
              <a href="../faq.php" class="footerLinks">FAQ</a><br>
              <a href="../contact_us.php" class="footerLinks">Contact Us</a><br>
              <a href="../specials.php" class="footerLinks">Member Specials</a><br>
             <a href="http://online.qvc.co.za/login/auth" class="footerLinks">Member Login</a> 
            <!--   <a href="portal-notice.php" class="footerLinks">Member Login</a>-->
              <br>
                <a href="http://online.qvc.co.za/login/auth" class="footerLinks">Book Online</a>
             <!-- <a href="portal-notice.php" class="footerLinks">Book Online</a>-->
          </p>
        </div>
        <div class="explore-column" style="background-color:#c4c4c4;">
          <!-- <h3 class="greyFooter">QVC CLUB DESTINATIONS</h3> -->
          <p class="footerText">
            <a href="baywater_village.php" class="footerLinks">Baywater Village</a><br>
            <a href="caribbean_estates.php" class="footerLinks">Caribbean Estates</a><br>
            <a href="crystal_springs.php" class="footerLinks">Crystal Springs</a><br>
    <a href="doornkop.php" class="footerLinks">Doornkop Fish &amp; Wildlife<br> Reserve</a><br>
            <a href="durban_spa.php" class="footerLinks">Durban Spa</a><br>
            <!--<a href="garden_route_chalets.php" class="footerLinks">Garden Route Chalets</a><br/>-->
            <a href="kagga_kamma.php" class="footerLinks">Kagga Kamma</a><br>
            <a href="kridzil.php" class="footerLinks">Kridzil</a><br>
            <a href="kruger_park_lodge.php" class="footerLinks">Kruger Park Lodge</a>
        </p>
        </div>
        <div class="explore-column" style="background-color:#c4c4c4;">
          <p class="footerText">
            <a href="little_eden.php" class="footerLinks">Little Eden</a><br>
            <a href="mabalingwe_nature_reserve.php" class="footerLinks">Mabalingwe Nature Reserve</a><br>
            <a href="magaliespark.php" class="footerLinks">Magalies Park</a><br>
            <a href="manzi_monate.php" class="footerLinks">Manzi Monate</a><br>
            <a href="mnarani.php" class="footerLinks">Mnarani</a><br>
            <a href="modumela_lodge.php" class="footerLinks">Modumela Lodge</a><br>
            <a href="monateng_safari_lodge.php" class="footerLinks">Monateng Safari Lodge</a><br>
            <a href="mount_amanzi.php" class="footerLinks">Mount Amanzi</a>
        </p>
        </div>
        <div class="explore-column" style="background-color:#c4c4c4;">
          <p class="footerText">
            <a href="ngwenya_lodge.php" class="footerLinks">Ngwenya Lodge</a><br>
            <a href="sandy_place.php" class="footerLinks">Sandy Place</a><br>
            <a href="st_michaels_sands.php" class="footerLinks">St Michaels Sands</a><br>
            <a href="sudwala_lodge.php" class="footerLinks">Sudwala Lodge</a><br>
            <a href="tortuga.php" class="footerLinks">Tortuga</a><br>
            <a href="uvongo.php" class="footerLinks">Uvongo</a>
        </p>
        </div>
        <div class="explore-column" style="background-color:#dfdfdf;">
            <p class="footerText" style="color:white;">
                <a href="../contact_us.php" class="footerContactLinks">Contact us</a><br>
                <a href="../privacy_policy.php" class="footerContactLinks">Privacy Policy</a><br>
                <a href="../terms_of_use.php" class="footerContactLinks">Terms &amp; Conditions</a></p>
                <p class="footerText">QVC Managing Agent</p>
                <a href="../vrs.php"><img src="../images/VRS_logo.png" width="150" height="54" border="0"></a><br>
        &nbsp;&nbsp;&nbsp;&nbsp; 
        </div>
    </div>
  </div>